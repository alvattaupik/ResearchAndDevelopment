...
Public Class frmRequestFullfillment
    ...
    Private Sub picBrowsBusinessPartner_Click(sender As Object, e As EventArgs) Handles picBrowsBusinessPartner.Click
        frmBrowseBusinessPartnerBAPPenyelesaian.ShowDialog()
    End Sub
    ...
    Private Sub PictureBox3_Click(sender As Object, e As EventArgs) Handles PictureBox3.Click
        frmBrowseConfigurationItems.ShowDialog()
    End Sub
    ...
    Private Sub btnProses_Click(sender As Object, e As EventArgs) Handles btnProses.Click
        ...
        If dgvListItem.RowCount = 0 Then
            DisplayPesanError("Isi Request Min 1 Baris", frmMainMenu.txtPesanError, 1000)
            Exit Sub
        End If

        Call AU_SISTER_SURAT("ADD-REQ")
    ...
    Sub AU_SISTER_SURAT(strFunction As String)
        ...
        Try
            ...
            Else
                ...
                For Me.intRow = 0 To dgvListItem.Rows.Count - 1
                    ...
                    AU_SISTER_DETAIL_LAMPIRAN(dgvListItem)
    ...
    Sub AU_SISTER_DETAIL_LAMPIRAN(dgvKode As DataGridView)

        Try
            KoneksiDB_EMAIL()
            Dim command As SqlCommand
            command = New SqlCommand("[AU_SISTER_DETAIL_LAMPIRAN_REQ]", KoneksiDBEmail)

            Dim adapter As New SqlDataAdapter(command)
            command.CommandType = CommandType.StoredProcedure
            command.Parameters.AddWithValue("NomorSurat", Trim(txtNoSurat.Text))
            command.Parameters.AddWithValue("Action", Trim(dgvKode.Rows(intRow).Cells(8).Value))
            ...
            command.Parameters.AddWithValue("Notifikasi", Trim(dgvKode.Rows(intRow).Cells(2).Value))
            command.Parameters.AddWithValue("KodeCabang", Trim(dgvKode.Rows(intRow).Cells(3).Value))
            command.Parameters.AddWithValue("FromDate", Trim(dgvKode.Rows(intRow).Cells(4).Value))
            command.Parameters.AddWithValue("ToDate", Trim(dgvKode.Rows(intRow).Cells(5).Value))
            command.Parameters.AddWithValue("Keterangan", Trim(dgvKode.Rows(intRow).Cells(6).Value))
            command.Parameters.AddWithValue("PathLampiran", Trim(dgvKode.Rows(intRow).Cells(7).Value))


            command.Parameters.Add("IDLampiran", SqlDbType.VarChar, 1000)
            command.Parameters("IDLampiran").Direction = ParameterDirection.Output
            ...
            GetExtentionfile((dgvKode.Rows(intRow).Cells(7).Value))
            CopyFileKeDirectoryAttachmentReq(command.Parameters("IDLampiran").Value.ToString(), Trim(dgvKode.Rows(intRow).Cells(7).Value))

            UpdatePathLampiran(MstrpathAttatchments, txtNoSurat.Text, command.Parameters("IDLampiran").Value.ToString())
    ...
    End Sub
    Sub UpdatePathLampiran(strPathLampiran As String, strNomorSurat As String, strIDLampiran As String)
        Try

            Call KoneksiDB_EMAIL()
            Dim str As String
            str = "UPDATE dbo.trxDetailReqFullfillment" & _
                  " SET PathLampiran='" & strPathLampiran & "'" & _
                  " WHERE NomorSurat='" & strNomorSurat & "' AND ID='" & strIDLampiran & "'"
    ...
    Private Sub picCancel_Click(sender As Object, e As EventArgs) Handles picCancel.Click
        ...
        If MsgBox("Apakah Yakin Anda Akan Membatalkan Surat Ini? Nomor Surat Tidak Dapat Digunakan Kembali", vbYesNo, "Konfirmasi?!") = vbYes Then
            ...
            Call AU_SISTER_SURAT("CANCEL-SJ")
    ...
    Private Sub cmbCabang_Click(sender As Object, e As EventArgs) Handles cmbCabang.Click
        Call KoneksiDB_EMAIL()
        LoadComboBox(cmbCabang, "SELECT KodeDivisi,NamaDivisi FROM dbo.V_Divisi", "KodeDivisi", "NamaDivisi", KoneksiDBEmail)
    End Sub
