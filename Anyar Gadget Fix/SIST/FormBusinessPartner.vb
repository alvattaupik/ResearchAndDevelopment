...
Public Class FormBusinessPartner
    ...
    Sub LoadDaftarVendor()
        ...
        Dim cmd As New SqlCommand("SELECT Top 10 CardName AS NamaVendor,isnull(Address,'') AS Alamat,isnull(Phone1,'') AS Telepon FROM RKM_LIVE_2.dbo.OCRD WHERE CardType='S' and RKM_LIVE_2.dbo.OCRD.CardName Like '%" & Trim(txtCariVendor.Text) & "%'", Koneksi1)
        cmd.CommandTimeout = 0
        Dim adapter As New SqlDataAdapter(cmd)
        Dim table As New DataTable
        adapter.Fill(table)
    ...
    Private Sub dgDaftarCabang_Click(sender As Object, e As EventArgs) Handles dgDaftarCabang.Click
        ...
        If MstrNamaForm = "Surat Terima" Then
